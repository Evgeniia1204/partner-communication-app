...
2. По умолчанию включены две точки: первая половина дня и вторая половина дня.
...
Timezone в MVP задаётся европейским вариантом `Europe/Berlin`.

В Telegram показывается короткая кнопка `Europe`; московский вариант не используется.
...
- Нужно ли позже расширять список timezone-вариантов за пределы `Europe`.
